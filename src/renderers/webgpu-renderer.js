...
export class WebGpuRenderer extends RendererBackend{
  ...
  setSource(pixels,width,height){if(!Number.isInteger(width)||!Number.isInteger(height)||width<1||height<1)throw new Error('WebGPU source dimensions must be positive integers');const count=width*height;if(!Number.isSafeInteger(count)||pixels?.length!==count*4)throw new Error('WebGPU source pixel length does not match its dimensions');const copy=new Uint8ClampedArray(pixels),token=++this.cancelVersion;const job=this.operationQueue.then(()=>this.configureSource(copy,width,height,token));this.operationQueue=job.catch(()=>{});return job}
