...
export class WebGpuRenderer extends RendererBackend{
  ...
  async pipelineFor(plan){
    if(this.pipelineCache.has(plan.key))return this.pipelineCache.get(plan.key);const device=await this.ensureDevice();device.pushErrorScope('validation');let pipeline,pipelineError=null,validationError=null;
    ...
    if(pipelineError?.name==='WGSLCompileError')throw pipelineError;if(validationError)throw new WebGPUValidationError(validationError.message);if(pipelineError)throw pipelineError;this.pipelineCache.set(plan.key,pipeline);return pipeline;
  }
