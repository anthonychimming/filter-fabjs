...
- The release build is `dist/filter-fabjs-v2.8.5.html`. Direct `file://` behavior can vary by browser security policy, so localhost remains the supported test path.
