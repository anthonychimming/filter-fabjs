...
const thumbnailService = fs.readFileSync('src/app/filter-thumbnail-service.js', 'utf8');
...
assert.match(app, /thumbnailService\.setSource\(state\.source,width,height\)/, 'new source images must prepare one bounded thumbnail source');
assert.match(app, /thumbnailService\.suspend\(\)[\s\S]*?rendererManager\.renderWithFallback[\s\S]*?thumbnailService\.resume\(\)/, 'authoritative main renders must suspend isolated thumbnail work');
assert.match(app, /requestThumbnail:requestLibraryThumbnail,clearThumbnailRequests:/, 'the Filter Library must receive explicit thumbnail request and cancellation callbacks');
assert.match(app, /getThumbnailDiagnostics:/, 'bounded thumbnail diagnostics must remain developer-facing through the read-only API');
assert.match(thumbnailService, /THUMBNAIL_MAX_DIMENSION=160/, 'thumbnail sources must be capped at 160 pixels');
assert.match(thumbnailService, /THUMBNAIL_CACHE_LIMIT=48/, 'the thumbnail LRU cache must be bounded');
assert.match(thumbnailService, /THUMBNAIL_CONCURRENCY=1/, 'thumbnail work must remain single-concurrency');
assert.match(thumbnailService, /sourceRevision[\s\S]*?cacheKey\(signature\)/, 'cache entries must include source revision and render signature');
...
assert.match(filterBrowser,/new IntersectionObserver\([\s\S]*?root:list,rootMargin:THUMBNAIL_ROOT_MARGIN/, 'visible and near-visible cards must drive thumbnail requests');
assert.match(filterBrowser,/rows\.slice\(0,THUMBNAIL_FALLBACK_COUNT\)/, 'browsers without IntersectionObserver must receive a bounded fallback batch');
assert.doesNotMatch(filterBrowser,/swatchHue|--swatch-hue/, 'decorative color swatches must no longer represent filter output');
assert.match(filterBrowser,/filter-thumbnail-image/, 'real thumbnail pixels must render into decorative card canvases');
assert.match(filterBrowser,/Preview unavailable for \$\{entry\.name\}/, 'thumbnail failure must have accessible nonfatal text');
...
assert.match(css,/\.filter-thumbnail\[data-thumbnail-state=ready\] \.filter-thumbnail-image\{opacity:1\}/, 'ready thumbnails must replace the neutral placeholder');
assert.match(css,/@media\(prefers-reduced-motion:reduce\)\{\.filter-thumbnail-image\{transition:none\}\}/, 'thumbnail fade-in must respect reduced-motion preference');
