...
async function run(url){
  ...
  try{
    ...
    input('#filterName','Dropdown draft');await selectPreset('builtin:duotone');await until(()=>$('#presetSelect').value==='builtin:duotone'&&ready());assert(!$('.filter-choice[open]')&&$('#filterName').value==='Duotone','dropdown selection immediately replaces a dirty draft without a warning dialog');
    ...
    input('[data-search]','Test Saved');$('.filter-results [data-entry-action="favorite"]').click();$('.filter-results [data-entry-action="load"]').click();await until(()=>!$('.filter-browser[open]')&&ready());assert(!$('.filter-choice[open]')&&$('#formulaR').value==='r','search selection immediately replaces a dirty draft without a warning dialog');
    ...
    const incoming={...portable,id:'incoming',name:'Incoming'};await importData(incoming);await until(()=>$('#filterName').value==='Incoming'&&ready());assert(!$('.filter-choice[open]')&&$('#savedDocumentStatus').textContent==='Imported · not saved','import immediately replaces a dirty draft and displays explicit unsaved identity');assert($('#presetSelect').value===''&&$('#presetSelect').selectedOptions[0].textContent.includes('Incoming'),'dropdown represents imported drafts without a false saved selection');assert(!JSON.parse(localStorage.getItem('ffw-custom-presets')).some(item=>item?.id==='incoming'),'import does not add a library record');
    $('#resetBtn').click();await until(()=>$('#presetSelect').value==='builtin:pass'&&ready());assert(!$('.filter-choice[open]')&&$('#filterName').value==='Pass Through','reset immediately replaces an imported draft without a warning dialog');
    await importData(incoming);await until(()=>$('#filterName').value==='Incoming'&&ready());
    ...
    const savedRecord=JSON.parse(localStorage.getItem('ffw-custom-presets')).find(item=>item?.id==='test-existing');await importData(savedRecord);await until(()=>$('#filterName').value===savedRecord.name&&ready());assert(!$('.filter-choice[open]'),'import replacement does not open the retired warning dialog');$('#savePresetBtn').click();await until(()=>$('.filter-choice[open]'));clickText('Save as new filter');await until(()=>$('.filter-choice h2')?.textContent==='Already saved');clickText('Use saved record');await until(()=>!$('.filter-choice[open]'));assert($('#savedDocumentStatus').textContent==='Saved','equal imported ID offers existing record without duplicating');
    ...
    input('#filterName','Keep deleted draft');$('#deletePresetBtn').click();await until(()=>$('.filter-choice[open]'));clickText('Delete and keep unsaved draft');await until(()=>!$('.filter-choice[open]'));assert($('#filterName').value==='Keep deleted draft'&&$('#savedDocumentStatus').textContent==='Not saved','delete can keep a dirty draft');assert(!JSON.parse(localStorage.getItem('ffw-custom-presets')).some(item=>item?.id==='test-existing'),'delete targets the loaded ID');assert(!localStorage.getItem('ffw-entry-v1:custom:test-existing'),'delete cleans only its own preferences');assert(![...$('#presetSelect').options].some(option=>option.value==='custom:test-existing'),'dirty deletion immediately removes the dropdown entry');
    await selectPreset(`custom:${copy.id}`);await until(()=>$('#presetSelect').value===`custom:${copy.id}`&&ready());$('#deletePresetBtn').click();await until(()=>$('.filter-choice[open]'));clickText('Delete');await until(()=>!$('.filter-choice[open]')&&$('#presetSelect').value==='builtin:pass'&&ready());assert(![...$('#presetSelect').options].some(option=>option.value===`custom:${copy.id}`),'clean deletion immediately removes the dropdown entry');$('#browseFiltersBtn').click();input('[data-search]','Other tab');assert(!doc.querySelector(`[data-entry-key="custom:${copy.id}"]`),'clean deletion immediately removes the search result');clickText('Close');
    input('#formulaR','r+');$('#resetBtn').click();await until(()=>$('#presetSelect').value==='builtin:pass'&&ready());assert(!$('.filter-choice[open]')&&$('#formulaR').value==='r'&&$('#filterName').value==='Pass Through','reset discards invalid edits without opening a warning dialog');
...
document.querySelector('#run').onclick=()=>run('../index.html');document.querySelector('#standalone').onclick=()=>run('../dist/filter-fabjs-v2.8.1.html');
